...
Serves a single-page web UI and a JSON REST API.
Runs in a background thread alongside the keyer loop and tkinter GUI.
"""
...
class WebServer:
    ...
    def __init__(self, *, get_status, get_config, update_config,
                 adjust_param, send_text, stop_send, test_tone,
                 scan_pins, port=80):
        self._get_status = get_status
        self._get_config = get_config
        self._update_config = update_config
        self._adjust_param = adjust_param
    ...
    def _make_handler(self):
        ...
        adjust_param = self._adjust_param
                ...
                if path == '/api/adjust':
                    try:
                        ...
                        if param and direction:
                            val = adjust_param(param, direction)
                            self._json_response({'ok': True, 'value': val})
                        else:
                            self._json_response({'error': 'need param and direction'}, 400)
                    except (json.JSONDecodeError, ValueError) as exc:
                        self._json_response({'error': str(exc)}, 400)
                    return
